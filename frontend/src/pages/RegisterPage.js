...
const PageWrapper = styled.div`
  ...
  background:
    ...
    url('https://images.unsplash.com/photo-1600210492493-0946911123ea?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1974&q=80')
...
const Alert = styled.div`
  background-color: ${({ success }) =>
    success ? 'rgba(46, 125, 50, 0.1)' : 'rgba(220, 53, 69, 0.1)'};
  color: ${({ success }) => (success ? 'var(--success-color)' : 'var(--error-color)')};
  padding: 1rem;
  border-radius: var(--radius-sm);
  margin-bottom: 1.5rem;
  border-left: 4px solid
    ${({ success }) => (success ? 'var(--success-color)' : 'var(--error-color)')};
  font-size: 0.95rem;
`;
...
const RegisterSchema = Yup.object().shape({
  firstName: Yup.string().required('Обязательное поле').min(2, 'Минимум 2 символа'),
  lastName: Yup.string().required('Обязательное поле').min(2, 'Минимум 2 символа'),
    ...
    <PageWrapper>
      <Container>
        ...
        {registerStatus.message && (
          <Alert success={registerStatus.success}>{registerStatus.message}</Alert>
        )}
          ...
          {({ isSubmitting }) => (
            <StyledForm>
              ...
              <FormGroup>
                ...
                <AdditionalInfo>
                  Не менее 6 символов, рекомендуется использовать буквы и цифры
                </AdditionalInfo>
              </FormGroup>
