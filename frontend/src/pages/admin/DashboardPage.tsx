...
const ChartOption = styled.button<ChartOptionProps>`
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchRecentBookings = async (): Promise<void> => {
      try {
        setBookingsLoading(true);
        const response = await bookingService.getAllBookings();
        
        // Преобразуем bookings из API к формату, который ожидает интерфейс
    ...
    const fetchStats = async (): Promise<void> => {
      try {
        ...
        setStats({
          ...
          bookings: response.data.stats.totalBookings || 0,
          occupancy: response.data.stats.occupancyRate || 0,
