...
interface ConnectionVariantResult {
  status?: number;
  ok: boolean;
  statusText?: string;
  error?: string;
...
const StatusIndicator = styled.span<StyledProps>`
...
const TestResult = styled.div<StyledProps>`
  margin-top: 0.5rem;
  padding: 0.5rem;
  background-color: ${props =>
    props.status === 'success'
...
const DiagnosticsPage: React.FC = () => {
  const [apiStatus, setApiStatus] = useState<'pending' | 'success' | 'warning' | 'error'>('pending');
  const [authStatus, setAuthStatus] = useState<'pending' | 'success' | 'warning' | 'neutral'>('pending');
  ...
  useEffect(() => {
    const getEnvInfo = async (): Promise<void> => {
      const info: EnvInfo = {
        nodeEnv: process.env.NODE_ENV || 'не указана',
        apiUrl: process.env.REACT_APP_API_URL || 'не указан',
  ...
  const runTest = async (testName: string, testFn: () => Promise<any>): Promise<void> => {
    try {
      ...
      setTestResults(prev => ({
        ...
        [testName]: {
          status: 'error',
          message: (error as Error).message || 'Ошибка выполнения теста',
  ...
  // Тест API-запросов
  const testApiEndpoints = async (): Promise<Record<string, EndpointTestResult>> => {
  ...
  // Тест конфигурации
  const testEnvironmentConfig = async (): Promise<Record<string, string | undefined>> => {
  ...
  const testConnectionVariants = async (): Promise<Record<string, ConnectionVariantResult>> => {
    const variants = [
    ...
    const results: Record<string, ConnectionVariantResult> = {};
  ...
  const testLocalStorage = async (): Promise<{ success: boolean; keysCount: number }> => {
