...
interface RoomGalleryProps {
  ...
  apiUrl?: string;
  optimizedImages?: Array<ResponsiveSources | null>;
}
...
const LightboxOverlay = styled.div<StyledProps>`
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
...
const RoomGallery: React.FC<RoomGalleryProps> = ({ images, apiUrl, optimizedImages = [] }) => {
  const [lightboxOpen, setLightboxOpen] = useState<boolean>(false);
  const [currentImage, setCurrentImage] = useState<number>(0);

  // Обрабатываем массив изображений, если он передан как строка
  let processedImages: string[] = Array.isArray(images) ? images : [];
  ...
  const formattedImages = processedImages.map(img => {
    if (apiUrl && typeof img === 'string' && !img.startsWith('http')) {
        ...
        {formattedImages.slice(1, maxThumbnails + 1).map((image, index) => (
          <ThumbnailContainer key={index} onClick={() => openLightbox(index + 1)}>
            <ImageWithFallback
              ...
              fallbackSrc="/images/room-placeholder.jpg"
              height="100%"
              sources={optimizedImages[index + 1]}
            />
          </ThumbnailContainer>
        ))}
